...
- [x] 2.1 新增四個 tally 標籤鍵（種類／形態／MEGA／招式）到純字串介面，中英各一份，切換語言時四個標籤跟著換。**驗證**：`tsc` 無錯；介面新增的鍵在中英兩份表都存在，缺一邊會是型別錯誤。
...
- [x] 5.2 型別檢查通過。**驗證**：`npm exec tsc -- --noEmit -p src/tsconfig.json` 離開碼為 0。
