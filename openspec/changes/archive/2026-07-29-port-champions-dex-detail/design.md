...
第一個切片（`port-champions-dex-foundation`）落地了資料層、兩個模式的 token、屬性字符與單張卡片；第二個（`port-champions-dex-grid`）把 208 張卡片放進 `<scroll-view>` 並在 iOS 實機驗過捲動。兩批都已 archive。
...
拆的判準是**輸入形狀不同**：`StatBars` 只需要六個數字，`AbilityList` 只需要特性槽，兩者都不需要知道種類或形態的存在。`FormSwitcher` 需要整個 `species`（它要列出所有形態並與基本形態比對屬性），所以它與另外兩者不同層級。
...
替代方案：讓 `t()` 的回傳變成 `string | readonly string[]` —— 每個呼叫點都要屬性斷言，`Strings` 介面提供的屬性安全就沒了。
...
### 屬性藥丸在 MODERN 填屬性色，在 POCKET 只留邊框
...
理由不是視覺偏好而是既有不變式：POCKET 的 UI 上色顏色數上限是四階灰，把屬性色畫進 POCKET 會直接違反 `retro-theme` 的要求。
...
設計稿在四處用 `title`：屬性藥丸的屬性中文名、形態鈕的屬性組合、招式的另一語言名、傷害類別全名。前兩者的資訊在藥丸與鈕的文字上本來就看得到（藥丸在中文模式顯示「草 Grass」），後兩者屬於招式表（批次 B）。所以這批直接不做 title 的替代物，不是延後。

設計稿的 `sp.f.forEach(f => { const i = new Image(); i.src = ... })` 預熱其他形態的圖像 —— 平台沒有 `Image` 建構子，移除。副作用是切換形態時大圖會有一次載入延遲；`SpeciesCard` 已經有的「佔位圖蓋在上面、載入成功才移除」機制同樣用在大圖上，所以延遲期間看到的是屬性字符而不是空白。
...
- 種類有多於一個形態時出現形態切換器；點另一個形態，大圖、形態標題、屬性藥丸、屬性清單、種族值、特性全部換成該形態的，而**捲動位置不動**
- 形態鈕只在該形態的屬性組合與基本形態不同時，才帶屬性字符
...
- 大圖載入失敗：與卡片一致 —— 佔位（該形態第一屬性的字符）從一開始就蓋在上面，圖像 `@load` 才移除。平台的 `@error` 在 native 不會觸發（§12.6），所以不依賴它
...
- 屬性字符在詳情藥丸與形態鈕兩個新表面上都可見（用對比計算驗證，不靠目視）
