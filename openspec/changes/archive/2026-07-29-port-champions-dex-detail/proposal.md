...
- 新增詳情面板：覆蓋層、標題列（名稱／副名稱／編號・世代・形態數・分類）、192px 大圖與形態標題、屬性藥丸與四項屬性清單、陣容與近似圖的警語、種族值列、特性列
- 新增形態切換器：依「基本／形態／地區形態／MEGA」分組，只在該形態改變屬性時才在鈕上蓋屬性字符
...
- `form-switcher`: 形態切換器 —— 分組順序、選中態、屬性字符只在改變屬性時出現的判準、以及切換形態時面板內容與捲動位置的行為
