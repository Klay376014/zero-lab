...
- [x] 4.4 跑完三項檢查並記錄驗收結果：pnpm run check、pnpm run typecheck、pnpm test 全綠；3.1、3.3、3.4 列出的手動項逐條回報。驗證：三個指令的輸出貼在對話中，各手動項逐條回報通過或失敗；任何一項失敗要停下來而不是繼續。
  ...
  **2026-08-12 實際結果**：三項自動化檢查全綠（check 四項不變式 0 violation、typecheck exit 0、test 221 passed）。手動項先以網頁預覽驗收，三個篩選條件皆有效。**其後同日以 `src/ios/Zero Lab` 的原生外殼上實機補驗，四項全數通過**：（一）切換顯示模式後原生搜尋框重繪配色（3.1）；（二）零結果句子出現、無列渲染、console 無輸出（3.3）；（三）深處捲動後篩到少數列的邊緣（3.4）；（四）有／無條件下各捲完整段無空白、無名稱與數字錯配（3.4）。第（三）(四)項同時是 `MOVE_INDEX_VIEWPORT` 維持 640 這個決定的實測驗證。
