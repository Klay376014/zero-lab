# move-query Specification

## Purpose

What the moves tab is asked for, and what it answers with. Covers the three conditions — a name search, a type selection and a damage class selection — held as shared reactive state that each is settable without disturbing the other two and that only a reset returns to its initial values, state that outlives the tab and is separate from the dex tab's query so that clearing one leaves the other alone, a search corpus of both of a move's names and nothing else with the type and the description text deliberately excluded, whitespace tokenisation in which every token must be found and a whitespace-only string imposes no condition, selections within a condition combining disjunctively while the three conditions combine conjunctively and an empty selection admits every move, and a derived sequence that names each move together with its index in the shared move table taken before any condition is applied.

Separate from `dex-query` rather than an extension of it. That capability's result pairs a species with the form its card displays, and its sort orders and Mega-only and multi-form-only filters are statements about species; the move table has no form and no species to answer them with. The reverse holds too: a damage class means nothing to a species.

The index a result carries is the shared move table's own, not a position in the result. `move-index` requires the move a row opens to be the row's move, and a position in a filtered sequence names a different move under every condition — so the source index is what lets that requirement hold once the sequence can be narrowed.
