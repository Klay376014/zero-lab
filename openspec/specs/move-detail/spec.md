...
It deliberately displays no move flags. `dex-data` requires the dataset to carry them and this capability requires the interface to leave them unread: 71 of the 496 moves carry none because the upstream source never recorded them, which is not the same statement as those moves lacking the properties, and no display can separate the two without a decision that has not been made.
