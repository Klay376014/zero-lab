# form-switcher Specification

## Purpose

How a species' other forms are reached from an open detail panel. Covers the switcher being absent rather than inert for a single-form species, grouping by form kind in a fixed order so a species with twenty cosmetic forms stays navigable, marking a button with type marks only when that form actually retypes the species, the selected state's visibility and the source-order rule that keeps it from being cancelled by its own base, and replacing the panel's content on a form change without moving — or touching — the scroll position.
