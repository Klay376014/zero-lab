...
> 最後更新：2026-07-29（詳情面板切片完成 —— 覆蓋層在實機成立見 §12.15，偏離清單見 §12.16。
> 尚未移植的只剩招式表）
...
| 詳情面板的 hover 提示（型別名、另一語言的招式名） | 觸控裝置無 hover | **移植版已處理，做法與這裡原本的預期不同**：型別名與形態鈕的型別直接寫進藥丸與鈕本身（中文模式的藥丸顯示「草 Grass」），不做點擊展開 —— 為兩個詞加一個展開狀態不划算。招式的另一語言名屬批次 B。見 §12.16 |
...
| 設計稿 | 這裡的做法 | 為什麼 |
|---|---|---|
| `.panel{overflow:auto}` 疊 `.mvwrap{max-height:400px;overflow:auto}` | 面板只有**一個** `<scroll-view>` | 巢狀同向捲動會搶同一個手勢，而 `dex-grid` 的 spec 已立下「不得有巢狀捲動容器」。招式表因此不再自己捲（批次 B 會沿用這個結構） |
