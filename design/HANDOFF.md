...
```
design/
  champions-dex.html          單檔自足，雙擊即可開啟
  champions-dex.json          完整資料集（231 種類 / 396 形態 / 511 招式 / 215 特性）
  pipeline/                   重建管線（見 §4）
  HANDOFF.md                  本文件
```

檔案含兩套內嵌字型（Silkscreen 8KB + Literata 62KB base64），這是 HTML 體積的主要來源。
檔案大小隨名單成長而變，這裡刻意不寫數字 —— 要知道就 `wc -c`。
...
| 不變式 | 目前值 | 若失敗代表 |
|---|---|---|
| 學習表分類成員數 | **231** | 遊戲更新了陣容 → 整份資料需重新檢視 |
| 可操作種類數 | **231** | 同上 |
| 形態條目數 | **396** | 新增／移除形態 |
| Mega 進化數 | **81** | 新增 Mega |
| 地區形態數 | **17** | |
| 時間戳相同下的招式機制衝突數 | **0** | 兩頁數值不同又無從分辨新舊 → 停下來看上游。**數值不同但時間戳不同不是錯誤**：聚合步驟取較新的那一頁（M-C 下有 Wish 與 Strength Sap 兩筆），並把裁決過程印出來 |
| Mega 中文命名規則不符數 | **0**（對 42 個 PokeAPI 實例） | 命名慣例變了 → §7 的推導不可再信任 |
| 地區形態規則驗證 | 重現 PokeAPI 的阿羅拉／伽勒爾 | 同上 |
| 每個形態的種族值完整度 | 6/6 | PokeAPI 缺資料 |
| 每個形態都有 sprite | 183/183 | 圖庫改路徑 |

**不再有排除項。** `#923 Pawmot` 曾存在於遊戲資料中但不可操作（只出現在選單的「對戰」圖示上），是 Bulbapedia 自稱 208 而解析出 209 的差異來源；1.2.0 起它成為正式可用，上游也把整個 `==Untransferable Pokémon==` 區段刪掉了。解析器原本拿那個標題當「其他形態」區段的結束樣式，**比對不到時不報錯、只是回傳空字串**，所以該節刪除會讓 87 筆形態安靜消失 —— 現在改為比對下一個第二層標題。
...
**Mega（81 個）** — 規則是 `超級` + 種類中文名（+ 全角Ｘ/Ｙ）。腳本會先對 PokeAPI 已localise 的 42 個 Mega 全部重算比對，**零不符才允許使用**。妙蛙花頁面沒有 Mega 的 langtable，推導是唯一途徑。
...
- 資料層六項不變式在載入期斷言，違反時丟錯而非靜默降級（231 種類 / 396 形態 / 81 Mega / 17 地區形態 / 511 招式 / 215 特性）
