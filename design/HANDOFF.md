...
> 最後更新：2026-07-30（招式表切片完成 —— 設計稿七項功能全部移植完畢，iOS 實機驗收已結清。
...
| 詳情面板的 hover 提示（型別名、另一語言的招式名、傷害類別全名） | 觸控裝置無 hover | **移植版已全部處理完畢，做法與這裡原本的預期不同**：型別名與形態鈕的型別直接寫進藥丸與鈕本身（中文模式的藥丸顯示「草 Grass」），不做點擊展開 —— 為兩個詞加一個展開狀態不划算。招式的另一語言名與傷害類別全名**移除且不補替代物**：前者透過語系鈕就整表切換，後者是三值封閉集合且同一列已有型別字符。見 §12.16 |
...
| `port-champions-dex-grid` | 208 筆網格、查詢列（搜尋／型別／世代／排序）、階梯揭示 | §12.13、§12.14 |
| `port-champions-dex-detail` | 覆蓋層、標題列、192px 大圖、型別藥丸、警語、種族值、特性、形態切換器 | §12.15、§12.16 |
...
**後續一個 bug 排除了它。** 世代與排序鈕的選中狀態完全看不出來，成因查出來是**普通的 CSS 級聯順序** —— `.ChipOn` 寫在 `.Chip` 上面，兩者同為單一 class 選擇器、權重相等，於是後者的 `background-color` 蓋掉前者。（型別鈕之所以正常，純粹因為那一對的順序剛好是對的。）
...
| 設計稿 | 這裡的做法 | 為什麼 |
|---|---|---|
| `.panel{overflow:auto}` 疊 `.mvwrap{max-height:400px;overflow:auto}` | 面板只有**一個** `<scroll-view>` | 巢狀同向捲動會搶同一個手勢，而 `dex-grid` 的 spec 已立下「不得有巢狀捲動容器」。招式表因此不再自己捲 —— 該切片已完成並沿用了這個結構，見本節後半與 §12.17 |
...
| 四處 `title` 提示（型別中文名、形態鈕型別、招式另一語言名、傷害類別全名） | 前兩者：資訊直接寫進藥丸與鈕（藥丸中文模式顯示「草 Grass」）。後兩者屬招式表 | 觸控裝置沒有 hover。**不是延後做替代物，是不需要** |
| `sp.f.forEach(f => new Image().src = …)` 預熱其他形態 | 移除 | 平台沒有 `Image` 建構子。切換形態時大圖會有一次載入延遲，期間顯示的是型別字符佔位而不是空白 |
| `.mvwrap{max-height:400px;overflow:auto}` 招式表自帶捲動區 | **不設高度上限、不設捲動容器**，105 列全部展開在面板既有的捲動容器裡 | 巢狀同向捲動搶同一個手勢，`dex-grid` 的 spec 已立下「不得有巢狀捲動容器」。代價是資料多的物種面板很長 —— 面板由內容決定高度，長內容就是長內容 |
| `.mvrow.hd{position:sticky;top:0}` 黏著表頭 | 表頭是流內的第一列，共用資料列的欄位 class | 表格沒有自己的捲動區可以黏。共用 class 是欄位對齊的來源，不是巧合 |
...
| 招式列 `img.title = typeName(...)`、`.mn` 的另一語言名、`.dc` 的傷害類別全名三處 hover 提示 | **全部移除，不補替代物** | 觸控裝置沒有 hover。另一語言名透過既有的語系鈕就整表切換（web 實測：切換後 72 個名稱全換、列數欄數列高欄左緣全部不變），而設計稿選擇單一名稱欄的理由正是兩種語言在長表上太吵；傷害類別是三值封閉集合且同一列已有型別字符。**不是延後做替代物，是不需要** |
...
| 新增什麼 | `scripts/check-contrast.mjs`。它**從主題原始碼讀出調色盤**（18 個型別色、4 階灰、10 個 MODERN token），不另存一份副本 —— 副本會漂移，而讀到過期副本的檢查比沒有檢查更糟 |
...
| 只算真的會渲染的組合 | POCKET × typechip **被排除**：型別鈕只在准許花型別色的模式下存在，POCKET 的篩選鈕維持平面表面、字符走 `surface`。量一個介面不會渲染的組合，與量錯背景是同一種不誠實 |
| 已驗證它會失敗 | 把毒的型別色改暗 → 三個組合各自被指名（含填色與背景 hex）；在 `GlyphSurface` 加一個假成員 → 被指名為未量測。兩項都在改回後恢復通過 |
...
**下限從 2.9 降為 2.5，這是刻意的。** 本系列的底色是次表面，MODERN 在它上面有三個型別低於舊下限：毒 2.53、龍 2.71、幽靈 2.89。這延續 §11 已載明的取捨（MODERN 把型別色畫在卡片表面時，毒／龍／幽靈／惡四個型別在 3.5 以下，那是「MODERN 就是要花顏色」的選擇），而**星號是不依賴顏色的第二訊號**，所以本系與否在對比最差的型別上仍讀得出來。要改回 2.9 就得拿掉本系列的底色，而底色是「一眼掃出哪些是本系」的來源。
...
UI 上那顆鈕的中文標籤是 `★ 屬修`（型別修正），不是更直譯的「本系」。**文件與程式註解仍然把這個概念叫「本系加成」** —— 只有控制項用短標籤，因為四個字的概念名放不進三顆排序鈕旁邊。英文標籤是 `★ STAB`。

若日後要統一用詞，`openspec/LANGUAGE.md`（CLAUDE.md 指定的詞彙表）目前不存在，這會是它的第一個條目。
...
**Android 依專案當前決定不在範圍內**，§10 的兩筆 Android 掛帳維持原狀。

### 12.18 佔位字承諾了規格沒要求的東西（2026-07-30，已修）

搜尋框的佔位字寫著「名稱 / 編號 / 型別 / 形態」，而實作**只比對兩個物種名**。搜 `475` 得到零結果，搜 `dragon`、`mega`、`阿羅拉` 也一樣。
...
所以既有的 spec 檢查、樣式檢查、型別檢查、iOS 驗收表全都不會抓到 —— **缺的東西從來沒有被要求過**，而唯一說出它應該存在的地方是一句 UI 文案。
...
改法是先扣掉**物種兩個名字已經滿足的詞**，剩下的才拿去比對形態，然後四層優先序：形態標籤 → 型別名（**全等**比對，否則「龍」會把任何含龍的詞當型別）→ 型別鈕 → 基本形態。
...
| `dragon` | 超級噴火龍Ｘ（火／龍） | `dragon` 不在名字裡，只能是型別 |
| `龍` | 基本形態 | 「龍」在「噴火龍」裡，被扣掉了 |

`dragon` 與 `龍` 顯示不同的形態不是 bug。**查詢最具體指認了什麼，卡片就回答什麼** —— 前者只可能是型別，後者是這個物種名字的一部分。

順帶一個實測數字：`龍` 命中 25 筆而不是 19 筆（型別鈕 Dragon 是 19 筆），多出的六筆是中文名含「龍」但不是龍屬性的暴鯉龍、化石翼龍、戰槌龍、護城龍、龍頭地鼠、冰雪巨龍。型別中文名與物種中文名在同一個語料裡，而部分名稱比對是既有需求 —— 這是**預期值**，已寫進 spec 的 Example，不是待修項。
